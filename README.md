...
Each state has a "lean" value L, based on the consensus of several experts (Cook, Sabato, et al.). L=0 means
...
experts' opinions. During July-September of 2020, these expert opinions seem to predict that a lot of states will behave
...
that the total number of electoral votes is wrong. Maine and Nebraska are broken down
into smaller parts that are either rated safe for one party or treated as independent
entities with their own randomness.
...
Positive = democratic. These are based on ratings by experts, including the following. The full
list is given in data.csv, which can be viewed or edited in a spreadsheet program.
...
The final three columns of the spreadsheet are date-stamped compilations of these 
ratings. These columns are not actually used by the program. They're there to make
...
The cut-offs for rules 2 and 3 are set in the code at the top of polls.py. Polls that satisfy these criteria are all weighted
based on how old they are, with the weights falling off exponentially with time.
...
the most recent one. Sometimes a pollster will release several analyses of the same poll on the same day, and if
this happens, we arbitrarily use the first one in the file.

To update the polls.csv file from an existing database, do "make polls" 
To download a new copy of the database and then update, do "make dl_polls" (only works on linux).
...
k = offset to lean[] values; I currently have this set to -0.6 by default as an estimate of the net votes lost to Biden in 2020 due to rejected mail-in ballots
...
very small) probability distribution for values arbitrarily close |y|=100. (To see this, note that a Cauchy
random variable can be generated by taking the tangent of a uniform
